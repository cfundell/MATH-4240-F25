...
matrix new_matrix_f(const int rows, const int cols)
{
   matrix mat;
   mat.rows = rows;
   mat.cols = cols;
   assert(rows>0);
   assert(cols>0);
   mat.val = (float*)malloc(sizeof(float)*rows*cols);
...
// Assumes that second matrix (B) has a number of rows less than A
matrix matrix_add_rows(const matrix* A, const matrix* B, const int starting_row, const int row_num) {
   const int rows = A->rows;
   const int cols = A->cols;
   assert(cols==B->cols);
   matrix C = new_matrix(rows, cols);
   ...
   for (int i = 1; i <= rows; i++) {
      for (int j=1; j<=cols; j++) {
         if (i < starting_row || i >= (starting_row + row_num)) { 
            mget(C, i, j) = mgetp(A, i, j);
         }
         else {
            mget(C, i, j) = mgetp(A, i, j) + mgetp(B, i - starting_row + 1, j);
         }
      }
   }
   return C;
...
//*
double poweriteration(vector *v, matrix *A, double TOL, int MaxIters)
{
  /*
  ...
  Input:
    ...
    A: the matrix, matrix
    TOL: toloerance for checking convergence (difference between succesive iterations), double
    MaxIters: max # of iterations allowed
  Output: 
  ...
  */
  ...
  vector Av = new_vector(rows);

  //normalize v
  double normv = sqrt(vector_dot_mult(v,v));
  for(i=0;i<size;i++){       
     v->val[i] = v->val[i]/normv;
  } 
  //Rayleigh quotient of v
  Av = matrix_vector_mult(A, v);    
  lambda = vector_dot_mult(&Av,v);
  ...
  while(1){
    ...
    Av = matrix_vector_mult(A, v);
    normv = sqrt(vector_dot_mult(&Av,&Av));
    for(i=0;i<size;i++){       
       v->val[i] = Av.val[i]/normv;
    } 
    
    Av = matrix_vector_mult(A, v);
    lambda = vector_dot_mult(&Av,v);
    
    printf("iter = %4d : Approx Eval = %lf : Err = %.16e\n",k,lambda, fabs(lambda-lambdaold));
    
    if(fabs(lambda-lambdaold)<TOL || k == MaxIters){
      break;
    }
  }
...
//*/
//*
double inverseiteration(double mu, vector *v, matrix *A, double TOL, int MaxIters)
{
  double lambda, lambdaold;
  int k=0;
  int i,j;
  ...
  for(i=1;i<=rows;i++){
     for(j=1;j<=cols;j++){
        mget(AA,i,j) = mgetp(A,i,j) - mu*(i==j);
     }
  } 
  ...
  free(Av.val); free(AA.val);
  return lambda;

}
//*/
//*
double rayleighquotieniteration(vector *v, matrix *A, double TOL, int MaxIters)
{
  double lambda, lambdaold;
  int k=0;
  int i,j;
  ...
  while(1){
    ...
    for(i=1;i<=rows;i++){
       for(j=1;j<=cols;j++){
          mget(AA,i,j) = mgetp(A,i,j)-lambdaold*(i==j);
       }
    } 
    Av = solve(&AA, v);
    normv = sqrt(vector_dot_mult(&Av,&Av));
